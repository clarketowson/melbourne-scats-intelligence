#!/usr/bin/env python3
r"""
generate_weekday_weekend_split_chunkedV3.py

V3 patched weekday/weekend split generator.

Based on generate_weekday_weekend_split_chunkedV2.py, with the key safety fixes from
...
- Uses defensive CSV parsing for existing weekday_weekend_split.csv.
...
Outputs:
    A:\TrafficAnalytics\PROJECTS\reports\deduped\weekday_weekend_split.csv
...
MONTHLY_CSV = REPORT_DIR / "weekday_weekend_split.csv"
FINAL_JSON = REPORT_DIR / "weekday_weekend_split_final.json"

DATE_RANGE_START = date(2014, 1, 1)
DATE_RANGE_END = date(2026, 4, 7)

ROW_TYPE_MONTH_TOTAL = "weekday_weekend_month_total"
    ...
    if not MONTHLY_CSV.exists():
        with MONTHLY_CSV.open("w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ...
                "next_month_start",
                "month_total_volume",
                "month_weekday_volume",
                "month_weekend_volume",
                "month_weekday_days",
                "month_weekend_days",
...
def append_month_row(
    ...
    with MONTHLY_CSV.open("a", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            int(month_weekday_volume) if month_weekday_volume is not None else "",
            int(month_weekend_volume) if month_weekend_volume is not None else "",
            int(month_weekday_days) if month_weekday_days is not None else "",
            int(month_weekend_days) if month_weekend_days is not None else "",
...
def query_month_weekday_weekend_split(con: duckdb.DuckDBPyConnection, month: MonthWindow) -> tuple:
    ...
    day_counts AS (
        SELECT
            COUNT(DISTINCT CASE WHEN iso_dow BETWEEN 1 AND 5 THEN count_date END) AS month_weekday_days,
            COUNT(DISTINCT CASE WHEN iso_dow IN (6, 7) THEN count_date END) AS month_weekend_days
        FROM base
    ),
    sums AS (
        SELECT
            SUM(volume_15m) AS month_total_volume,
            SUM(CASE WHEN iso_dow BETWEEN 1 AND 5 THEN volume_15m ELSE 0 END) AS month_weekday_volume,
            SUM(CASE WHEN iso_dow IN (6, 7) THEN volume_15m ELSE 0 END) AS month_weekend_volume
        FROM base
    )
    SELECT
        sums.month_total_volume,
        sums.month_weekday_volume,
        sums.month_weekend_volume,
        day_counts.month_weekday_days,
        day_counts.month_weekend_days
    FROM sums
    CROSS JOIN day_counts
    """
    ...
    # SUM() returns NULL when the month has no rows. Treat that as an explicit
    # zero-row month so it is marked complete and not retried forever.
    if row[0] is None:
        return (None, None, None, None, None)
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    with MONTHLY_CSV.open("r", newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            month_total = (row.get("month_total_volume") or "").strip()

            if label in all_labels and (row_type == ROW_TYPE_ZERO_ROW_MONTH or month_total == ""):
                zero_row_months.append(label)

    zero_row_months = sorted(set(zero_row_months))

    base_payload = {
        "metric_name": "weekday_weekend_split",
        ...
        "weekday_share": None,
        "weekend_share": None,
        "weekday_share_formatted": "N/A",
        "weekend_share_formatted": "N/A",
        "weekday_avg_day_volume": None,
        "weekend_avg_day_volume": None,
        "total_elapsed_seconds": 0.0,
        "generated_at_epoch": round(time.time(), 3),
        "weekday_weekend_split_csv": str(MONTHLY_CSV),
    }
    ...
    monthly AS (
        SELECT
            month_label,
            TRY_CAST(month_total_volume AS BIGINT) AS month_total_volume,
            TRY_CAST(month_weekday_volume AS BIGINT) AS month_weekday_volume,
            TRY_CAST(month_weekend_volume AS BIGINT) AS month_weekend_volume,
            TRY_CAST(month_weekday_days AS BIGINT) AS month_weekday_days,
            TRY_CAST(month_weekend_days AS BIGINT) AS month_weekend_days,
            TRY_CAST(month_elapsed_seconds AS DOUBLE) AS month_elapsed_seconds
        FROM raw_csv
        WHERE month_label IS NOT NULL
          AND month_label <> ''
          AND COALESCE(row_type, '{ROW_TYPE_MONTH_TOTAL}') <> '{ROW_TYPE_ZERO_ROW_MONTH}'
          AND month_total_volume IS NOT NULL
          AND month_total_volume <> ''
    ),
    totals AS (
        SELECT
            SUM(month_total_volume) AS total_volume,
            SUM(month_weekday_volume) AS weekday_volume,
            SUM(month_weekend_volume) AS weekend_volume,
            SUM(month_weekday_days) AS weekday_days,
            SUM(month_weekend_days) AS weekend_days
        FROM monthly
    ),
    elapsed AS (
    ...
    SELECT
        totals.total_volume,
        totals.weekday_volume,
        totals.weekend_volume,
        totals.weekday_days,
        totals.weekend_days,
        CASE
            WHEN totals.total_volume IS NULL OR totals.total_volume = 0 THEN NULL
            ELSE totals.weekday_volume * 100.0 / totals.total_volume
        END AS weekday_share,
        CASE
            WHEN totals.total_volume IS NULL OR totals.total_volume = 0 THEN NULL
            ELSE totals.weekend_volume * 100.0 / totals.total_volume
        END AS weekend_share,
        CASE
            WHEN totals.weekday_days IS NULL OR totals.weekday_days = 0 THEN NULL
            ELSE totals.weekday_volume * 1.0 / totals.weekday_days
        END AS weekday_avg_day_volume,
        CASE
            WHEN totals.weekend_days IS NULL OR totals.weekend_days = 0 THEN NULL
            ELSE totals.weekend_volume * 1.0 / totals.weekend_days
        END AS weekend_avg_day_volume,
    ...
    if row:
        base_payload.update({
            "total_volume": row[0],
            "weekday_volume": row[1],
            "weekend_volume": row[2],
            "weekday_days": row[3] if row[3] is not None else 0,
            "weekend_days": row[4] if row[4] is not None else 0,
            "weekday_share": row[5],
            "weekend_share": row[6],
            "weekday_share_formatted": fmt_pct(row[5]),
            "weekend_share_formatted": fmt_pct(row[6]),
            "weekday_avg_day_volume": row[7],
            "weekend_avg_day_volume": row[8],
            "total_elapsed_seconds": round(float(row[9]), 3) if row[9] is not None else 0.0,
        })
...
def print_final_payload(payload: dict) -> None:
    ...
    print(f"Running total volume : {fmt_int(payload['total_volume'])}", flush=True)
    print(f"Running weekday share: {payload['weekday_share_formatted']}", flush=True)
    print(f"Running weekend share: {payload['weekend_share_formatted']}", flush=True)
...
def main() -> None:
    ...
    print("=" * 90, flush=True)
    print("WEEKDAY/WEEKEND SPLIT V3 - ONE MONTH PER EXECUTION", flush=True)
    print("=" * 90, flush=True)
    ...
    try:
        ...
        (
            ...
            month_weekday_days,
            month_weekend_days,
        ) = query_month_weekday_weekend_split(con, next_month)
        ...
        month_weekday_share = (
            month_weekday * 100.0 / month_total
            if month_total not in (None, 0) and month_weekday is not None
            else None
        )
        month_weekend_share = (
            month_weekend * 100.0 / month_total
            if month_total not in (None, 0) and month_weekend is not None
            else None
        )
        ...
        print(f"Month total volume   : {fmt_int(month_total)}", flush=True)
        print(f"Weekday volume       : {fmt_int(month_weekday)}", flush=True)
        print(f"Weekend volume       : {fmt_int(month_weekend)}", flush=True)
        print(f"Weekday days         : {fmt_int(month_weekday_days)}", flush=True)
        print(f"Weekend days         : {fmt_int(month_weekend_days)}", flush=True)
        print(f"Weekday share        : {fmt_pct(month_weekday_share)}", flush=True)
        print(f"Weekend share        : {fmt_pct(month_weekend_share)}", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print("\nERROR: generate_weekday_weekend_split_chunkedV3.py failed.", file=sys.stderr, flush=True)
