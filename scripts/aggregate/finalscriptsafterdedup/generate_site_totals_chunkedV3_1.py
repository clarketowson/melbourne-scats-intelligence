...
V3.1 patched site totals generator.

Fixes / improvements over V3:
- Disables DuckDB Unicode progress bar to stop PowerShell mojibake output.
- Uses more defensive CSV parsing for existing site_totals.csv.
- Writes zero-row month marker rows so months like 2018-12 do not loop forever.
- Excludes marker rows from site totals aggregation.
...
def connect_db() -> duckdb.DuckDBPyConnection:
    ...
    # DuckDB Unicode block characters as Ôû... mojibake and can obscure errors.
    try:
...
def ensure_site_totals_csv() -> None:
    ...
    # If an older V3 CSV exists without row_type, leave it alone.
    # DictReader handles missing row_type as None; final SQL handles both.
    # We don't rewrite the file while a process may be running.
...
def load_completed_months() -> set[str]:
    ...
    with SITE_TOTALS_CSV.open("r", newline="", encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        if not reader.fieldnames or "month_label" not in reader.fieldnames:
            raise RuntimeError(f"Invalid CSV header in {SITE_TOTALS_CSV}: {reader.fieldnames}")
        ...
        for row in reader:
            ...
            # A month is complete if at least one row exists for the month,
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    base_payload = {
        ...
        "rows_in_site_totals_csv": rows_in_csv,
        "distinct_sites": None,
        "top_site_id": None,
        "top_site_name": None,
    ...
    site_csv_path = str(SITE_TOTALS_CSV).replace(chr(92), "/")

    # all_varchar=true prevents DuckDB CSV auto-typing from choking on marker rows.
    sql = f"""
    WITH raw_csv AS (
        SELECT *
    ...
    site_names_raw AS (
        SELECT TRY_CAST(scats_site AS INTEGER) AS scats_site, site_name FROM scats_site
        UNION ALL
        SELECT TRY_CAST(scats_site AS INTEGER) AS scats_site, site_name FROM cont.scats_site
        UNION ALL
        SELECT TRY_CAST(scats_site AS INTEGER) AS scats_site, site_name FROM rec.scats_site
    ),
    site_names AS (
        SELECT
            scats_site,
    ...
    if row:
        base_payload.update({
            "distinct_sites": row[0],
            "top_site_id": row[1],
            "top_site_name": row[2],
            "top_site_total_volume": row[3],
            "grand_total_volume": row[4],
...
def print_final_payload(payload: dict) -> None:
    ...
    print(f"Rows in site CSV     : {fmt_int(payload['rows_in_site_totals_csv'])}", flush=True)
    print(f"Distinct sites       : {fmt_int(payload['distinct_sites'])}", flush=True)
    print(f"Top site             : {payload['top_site_id']} - {payload['top_site_name']}", flush=True)
...
def main() -> None:
    ...
    print("=" * 90, flush=True)
    print("SITE TOTALS V3.1 - ONE MONTH PER EXECUTION", flush=True)
    print("=" * 90, flush=True)
    ...
    print(f"Site totals CSV      : {SITE_TOTALS_CSV}", flush=True)
    ...
    try:
        ...
        if rows:
            month_best_site, month_best_volume = max(rows, key=lambda x: (x[1] if x[1] is not None else -1, -int(x[0])))
        ...
        print("\nERROR: generate_site_totals_chunkedV3_1.py failed.", file=sys.stderr, flush=True)
