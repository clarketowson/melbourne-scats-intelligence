#!/usr/bin/env python3
r"""
generate_time_bin_profile_chunkedV2.py

Builds network time-bin totals and average profile using the improved one-month-per-execution method.
...
- Writes time-bin totals for that month immediately
...
def load_completed_months() -> set[str]:
    ...
    with TIME_BIN_CSV.open("r", newline="", encoding="utf-8") as f:
...
def query_month_time_bin_totals(con: duckdb.DuckDBPyConnection, month: MonthWindow) -> tuple[list[tuple], int]:
    ...
    return rows, days_in_month_loaded
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    if not done_labels:
        return {
            ...
            "months_completed": 0,
            "is_complete": False,
            "rows_in_time_bin_profile_csv": 0,
    ...
    WITH tb AS (
        ...
        FROM read_csv_auto('{time_bin_csv_path}', header=true)
    ),
    totals AS (
        SELECT
            time_bin,
    ...
    day_totals AS (
        SELECT
            SUM(DISTINCT days_in_month_loaded) AS total_days_loaded
        FROM (
            SELECT month_label, MAX(days_in_month_loaded) AS days_in_month_loaded
            FROM read_csv_auto('{time_bin_csv_path}', header=true)
    ...
    best AS (
        SELECT
            totals.time_bin,
            totals.total_time_bin_volume
        FROM totals
        ORDER BY totals.total_time_bin_volume DESC, totals.time_bin
        LIMIT 1
    ),
    elapsed AS (
    ...
    return {
        ...
        "rows_in_time_bin_profile_csv": row[0] if row else 0,
        "busiest_time_bin": row[1] if row else None,
        "busiest_time_bin_total_volume": row[2] if row else None,
        "total_days_loaded": row[3] if row else 0,
        "busiest_time_bin_average_daily_volume": row[4] if row else None,
...
def main() -> None:
    ...
    print("=" * 90)
    print("TIME BIN PROFILE RUN - ONE MONTH PER EXECUTION")
    print("=" * 90)
    ...
    print(f"Time-bin CSV output  : {TIME_BIN_CSV}")
    ...
    try:
        ...
        if rows:
            month_best_time_bin, month_best_volume = max(rows, key=lambda x: (x[1], x[0]))
        ...
        print(f"Days loaded          : {fmt_int(days_in_month_loaded)}")
        ...
        print(f"Top bin so far       : {final_payload['busiest_time_bin']}")
