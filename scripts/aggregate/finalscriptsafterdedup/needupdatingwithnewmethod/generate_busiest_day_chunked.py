#!/usr/bin/env python3
r"""
generate_busiest_day_chunked.py

Computes busiest day safely using monthly chunking.

Pipeline:
- build scats_all_clean_dedup in-session from the 3 scats_clean views
- for each month:
    aggregate SUM(volume_15m) by count_date
    save monthly day totals to CSV immediately
- after all months:
    combine monthly day totals
    compute final busiest day
...
def build_final_result(con: duckdb.DuckDBPyConnection) -> dict:
    ...
    return {
        "busiest_day": row[0] if row else None,
...
def main() -> None:
    ...
    try:
        ...
        for idx, month in enumerate(months, start=1):
            ...
            rows = query_month_day_totals(con, month)
        ...
        final_payload = {
            ...
            "busiest_day": str(final["busiest_day"]) if final["busiest_day"] is not None else None,
            "busiest_day_total_volume": final["busiest_day_total_volume"],
        ...
        print("RUN COMPLETE")
        print("=" * 90)
        print(f"Busiest day          : {final_payload['busiest_day']}")
        print(f"Day total volume     : {fmt_int(final_payload['busiest_day_total_volume'])}")
