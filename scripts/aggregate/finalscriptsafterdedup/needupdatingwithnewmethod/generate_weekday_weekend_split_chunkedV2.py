#!/usr/bin/env python3
r"""
generate_weekday_weekend_split_chunkedV2.py

Builds weekday vs weekend cleaned volume split using the improved one-month-per-execution method.
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    WITH monthly AS (
        SELECT
            ...
            CAST(month_weekday_volume AS BIGINT) AS month_weekday_volume,
            CAST(month_weekend_volume AS BIGINT) AS month_weekend_volume,
            CAST(month_weekday_days AS BIGINT) AS month_weekday_days,
            CAST(month_weekend_days AS BIGINT) AS month_weekend_days
    ...
    return {
        ...
        "weekday_volume": row[1] if row else None,
        "weekend_volume": row[2] if row else None,
        "weekday_days": row[3] if row else 0,
        "weekend_days": row[4] if row else 0,
        "weekday_share": row[5] if row else None,
        "weekend_share": row[6] if row else None,
        "weekday_share_formatted": fmt_pct(row[5]) if row else "N/A",
        "weekend_share_formatted": fmt_pct(row[6]) if row else "N/A",
        "weekday_avg_day_volume": row[7] if row else None,
        "weekend_avg_day_volume": row[8] if row else None,
        "total_elapsed_seconds": round(float(row[9]), 3) if row and row[9] is not None else 0.0,
        "generated_at_epoch": round(time.time(), 3),
        "weekday_weekend_split_csv": str(MONTHLY_CSV),
    }
...
def main() -> None:
    ...
    print("=" * 90)
    print("WEEKDAY/WEEKEND SPLIT RUN - ONE MONTH PER EXECUTION")
    print("=" * 90)
    ...
    try:
        ...
        print(f"Weekday volume       : {fmt_int(month_weekday)}")
        print(f"Weekend volume       : {fmt_int(month_weekend)}")
        print(f"Weekday days         : {fmt_int(month_weekday_days)}")
        print(f"Weekend days         : {fmt_int(month_weekend_days)}")
        print(f"Weekday share        : {fmt_pct(month_weekday_share)}")
        print(f"Weekend share        : {fmt_pct(month_weekend_share)}")
        ...
        print(f"Running weekday share: {final_payload['weekday_share_formatted']}")
        print(f"Running weekend share: {final_payload['weekend_share_formatted']}")
