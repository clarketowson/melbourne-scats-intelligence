#!/usr/bin/env python3
r"""
generate_busiest_time_bin_chunked.py

Computes busiest time bin safely using monthly chunking.

Pipeline:
- build scats_all_clean_dedup in-session from the 3 scats_clean views
- for each month:
    aggregate SUM(volume_15m) by time_bin
    save monthly time-bin totals to CSV immediately
- after all months:
    combine monthly time-bin totals
    compute final busiest time bin
...
def main() -> None:
    ...
    print("=" * 90)
    print("CHUNKED BUSIEST TIME BIN RUN")
    print("=" * 90)
    ...
    try:
        ...
        for idx, month in enumerate(months, start=1):
            ...
            rows = query_month_time_bin_totals(con, month)
        ...
        final_payload = {
            ...
            "busiest_time_bin": final["busiest_time_bin"],
            "busiest_time_bin_total_volume": final["busiest_time_bin_total_volume"],
        ...
        print("RUN COMPLETE")
        print("=" * 90)
        print(f"Busiest time bin     : {final_payload['busiest_time_bin']}")
        print(f"Time-bin total volume: {fmt_int(final_payload['busiest_time_bin_total_volume'])}")
