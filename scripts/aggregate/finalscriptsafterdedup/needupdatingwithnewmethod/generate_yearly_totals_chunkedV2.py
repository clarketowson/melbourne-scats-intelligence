#!/usr/bin/env python3
r"""
generate_yearly_totals_chunkedV2.py

Builds yearly traffic totals using the improved one-month-per-execution method.
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    return {
        ...
        "rows_in_yearly_totals_csv": row[0] if row else 0,
        "best_year": row[1] if row else None,
        "best_year_avg_daily_volume": row[2] if row else None,
        "lowest_year": row[3] if row else None,
        "lowest_year_avg_daily_volume": row[4] if row else None,
...
def main() -> None:
    ...
    print("=" * 90)
    print("YEARLY TOTALS RUN - ONE MONTH PER EXECUTION")
    print("=" * 90)
    ...
    try:
        ...
        print(f"Best year so far     : {final_payload['best_year']}")
        print(f"Lowest year so far   : {final_payload['lowest_year']}")
