#!/usr/bin/env python3
r"""
generate_site_totals_chunkedV2.py

Builds site total cleaned volume using the improved one-month-per-execution method.
...
- Writes site totals for that month immediately
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    WITH site_monthly AS (
        ...
        FROM read_csv_auto('{site_csv_path}', header=true)
    ),
    site_totals AS (
        SELECT
            scats_site,
    ...
    return {
        ...
        "rows_in_site_totals_csv": row[0] if row else 0,
...
def main() -> None:
    ...
    print("=" * 90)
    print("SITE TOTALS RUN - ONE MONTH PER EXECUTION")
    print("=" * 90)
