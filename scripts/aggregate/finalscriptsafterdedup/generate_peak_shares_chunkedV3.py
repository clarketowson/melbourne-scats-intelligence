#!/usr/bin/env python3
r"""
generate_peak_shares_chunkedV3.py

Computes AM and PM peak shares using the improved one-month-per-execution method.

V3 carries across the reliability fixes from generate_busiest_time_bin_chunkedV3.py:
...
def append_month_row(
    ...
    if month_total_volume in (None, 0):
        am_share = None
        pm_share = None
    else:
        am_share = (month_am_volume * 100.0 / month_total_volume) if month_am_volume is not None else None
        pm_share = (month_pm_volume * 100.0 / month_total_volume) if month_pm_volume is not None else None
    ...
    rows_to_write = [
        [
            ...
            round(float(am_share), 6) if am_share is not None else "",
            round(float(pm_share), 6) if pm_share is not None else "",
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    if not done_labels:
        return {
            ...
            "is_complete": False,
            "total_volume": None,
            "am_window_start": AM_START,
            "am_window_end_exclusive": AM_END,
            "pm_window_start": PM_START,
            "pm_window_end_exclusive": PM_END,
            "am_volume": None,
            "pm_volume": None,
            "am_peak_share": None,
            "pm_peak_share": None,
...
def main() -> None:
    ...
    print("=" * 90)
    print("CHUNKED PEAK SHARES RUN V3 - ONE MONTH PER EXECUTION")
    print("=" * 90)
    ...
    try:
        ...
        csv_rows_written = append_month_row(next_month, month_total, month_am, month_pm, month_elapsed)
        ...
        print(f"Running AM volume    : {fmt_int(final_payload['am_volume'])}")
        print(f"Running PM volume    : {fmt_int(final_payload['pm_volume'])}")
