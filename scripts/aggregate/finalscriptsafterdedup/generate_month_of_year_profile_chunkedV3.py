#!/usr/bin/env python3
r"""
generate_month_of_year_profile_chunkedV3.py

V3 patched month-of-year traffic profile generator.

Based on generate_month_of_year_profile_chunkedV2.py, with the key safety fixes from
...
- Uses defensive CSV parsing for existing month_of_year_profile.csv.
...
ROW_TYPE_MONTH_TOTAL = "month_of_year_month_total"
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    monthly AS (
        SELECT
            TRY_CAST(month_of_year AS INTEGER) AS month_of_year,
    ...
    if row:
        base_payload.update({
            "rows_in_month_of_year_profile_csv": row[0] if row[0] is not None else 0,
            "best_month_of_year": row[1],
            "best_month_name": row[2],
            "best_month_avg_daily_volume": row[3],
            "lowest_month_of_year": row[4],
            "lowest_month_name": row[5],
            "lowest_month_avg_daily_volume": row[6],
...
def print_final_payload(payload: dict) -> None:
    ...
    print(f"Rows in profile CSV  : {fmt_int(payload['rows_in_month_of_year_profile_csv'])}", flush=True)
    print(f"Physical CSV rows    : {fmt_int(payload['physical_rows_in_csv'])}", flush=True)
    print(f"Best month so far    : {payload['best_month_name']} ({fmt_int(payload['best_month_avg_daily_volume'])} avg/day)", flush=True)
    print(f"Lowest month so far  : {payload['lowest_month_name']} ({fmt_int(payload['lowest_month_avg_daily_volume'])} avg/day)", flush=True)
...
def main() -> None:
    ...
    print("=" * 90, flush=True)
    print("MONTH OF YEAR PROFILE V3 - ONE MONTH PER EXECUTION", flush=True)
    print("=" * 90, flush=True)
    ...
    try:
        ...
        print(f"Month name           : {month_name}", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print("\nERROR: generate_month_of_year_profile_chunkedV3.py failed.", file=sys.stderr, flush=True)
