# run_generate_weekday_weekend_split_chunkedV3_until_done.ps1
#
# Relaunches generate_weekday_weekend_split_chunkedV3.py until all 148 months are complete.
# Designed for the one-month-per-execution V3 script.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\run_generate_weekday_weekend_split_chunkedV3_until_done.ps1

$ErrorActionPreference = "Stop"

$PythonExe = "A:\TrafficAnalytics\PROJECTS\env\Scripts\python.exe"
$Script   = "A:\TrafficAnalytics\PROJECTS\scripts\finalscriptsafterdedup\generate_weekday_weekend_split_chunkedV3.py"

$ReportDir = "A:\TrafficAnalytics\PROJECTS\reports\deduped"
$FinalJson = Join-Path $ReportDir "weekday_weekend_split_final.json"
$LogDir    = Join-Path $ReportDir "logs"
$LogFile   = Join-Path $LogDir "weekday_weekend_split_chunkedV3_wrapper.log"
...
Log-Line "Starting weekday/weekend split V3 wrapper"
...
while ($true) {
    ...
    $summary = Get-ProgressSummary
    if ($summary) {
        Log-Line "Progress: $($summary.months_completed)/$($summary.months_total) months complete; weekday_share=$($summary.weekday_share_formatted); weekend_share=$($summary.weekend_share_formatted)"
        if ($summary.zero_row_months) {
            Log-Line "Zero-row months recorded: $($summary.zero_row_months -join ', ')"
        }
