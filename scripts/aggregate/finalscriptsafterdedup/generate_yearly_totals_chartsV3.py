# generate_yearly_totals_charts_v3.py
...
OUT_DIR = Path(r"A:\TrafficAnalytics\PROJECTS\reports\deduped\charts\yearly_totals_v3")
...
def configure_axes(ax):
    ax.grid(axis="y", alpha=0.32, color=COLOUR_GRID)
...
def title_block(fig, title, subtitle=None):
    fig.suptitle(title, fontsize=25, weight="bold", y=0.965)
...
def savefig(fig, name):
    fig.subplots_adjust(top=0.86, bottom=0.16, left=0.075, right=0.975)
...
def add_2018_annotation(ax, x_value, y_value, y_offset=30):
    if zero_row_months:
        ax.annotate(
            ingest_note_short,
            xy=(x_value, y_value),
            ...
            arrowprops=dict(arrowstyle="->", color=COLOUR_WARNING, lw=1.1)
        )
...
if not row_2018.empty:
    add_2018_annotation(ax, 2018, row_2018["total_billion"].iloc[0], y_offset=34)

footer(fig, standard_footer())
savefig(fig, "yearly_total_vehicle_movements_v3.png")
...
row_2018 = yearly[yearly["year"] == 2018]
if not row_2018.empty:
    add_2018_annotation(ax, 2018, row_2018["avg_daily_million"].iloc[0], y_offset=34)

footer(fig, standard_footer())
savefig(fig, "yearly_average_daily_movements_v3.png")
...
savefig(fig, "yearly_yoy_percentage_change_v3.png")
...
if not row_2018.empty and zero_row_months:
    add_2018_annotation(ax, 2018, row_2018["index_base_100"].iloc[0], y_offset=34)

footer(fig, standard_footer("Index chart uses full calendar years only. Partial 2026 excluded."))
savefig(fig, "yearly_index_growth_full_years_v3.png")
...
savefig(fig, "yearly_cumulative_vehicle_movements_v3.png")
...
savefig(fig, "yearly_covid_low_vs_record_year_v3.png")
...
footer(fig, standard_footer())
savefig(fig, "yearly_months_loaded_v3.png")
...
summary_path = OUT_DIR / "yearly_totals_summary_for_web_v3.csv"
yearly.to_csv(summary_path, index=False)

print("Created yearly totals V3 chart pack:")
