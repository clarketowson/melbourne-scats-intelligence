...
def _candidate_from_table(raw: pd.DataFrame, source_name: str, sheet_name: str, header_row: Optional[int]) -> Optional[pd.DataFrame]:
    ...
    if header_row is not None:
        headers = [_clean_header_value(v) or f"col_{i}" for i, v in enumerate(raw.iloc[header_row].tolist())]
        ...
        if site_col is None and columns:
            site_col = max(columns, key=lambda c: work[c].map(_looks_like_site_id).sum())

        name_col = None
        for candidate in [
            "site_name", "site_description", "description", "location", "location_name",
        ...
        if lat_col is None and columns:
            lat_col = max(columns, key=lambda c: work[c].map(_looks_like_lat).sum())
            if work[lat_col].map(_looks_like_lat).sum() < 10:
                lat_col = None
        if lon_col is None and columns:
            lon_col = max(columns, key=lambda c: work[c].map(_looks_like_lon).sum())
            if work[lon_col].map(_looks_like_lon).sum() < 10:
                lon_col = None
    else:
        work = raw.copy()
        columns = list(work.columns)
    ...
    site_count = work[site_col].map(_looks_like_site_id).sum() if site_col is not None else 0
    if site_col is None or site_count < 20:
        return None

    out = pd.DataFrame(index=work.index)
    out["site"] = work[site_col].map(format_site_id)
    out = out[out["site"].map(lambda x: isinstance(x, str) and re.fullmatch(r"\d+", x) is not None)].copy()
    if out.empty:
        return None

    if name_col is not None:
        out["site_name"] = work.loc[out.index, name_col].astype(str).str.strip()
        out.loc[out["site_name"].str.lower().isin(["", "nan", "none", "null"]), "site_name"] = np.nan
    else:
        out["site_name"] = np.nan

    out["latitude"] = pd.to_numeric(work.loc[out.index, lat_col], errors="coerce") if lat_col is not None else np.nan
    out["longitude"] = pd.to_numeric(work.loc[out.index, lon_col], errors="coerce") if lon_col is not None else np.nan
...
def chart_busiest_sites(
    ...
    manifest.append({
        ...
        "axis_fix": "V3.5 scans all Excel sheets, selects the largest valid SCATS site table, labels sites with readable names, and preserves integer-only site IDs",
    })
