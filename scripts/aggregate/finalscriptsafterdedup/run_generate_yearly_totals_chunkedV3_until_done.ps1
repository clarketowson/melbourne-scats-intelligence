# run_generate_yearly_totals_chunkedV3_until_done.ps1
#
# Relaunches generate_yearly_totals_chunkedV3.py until all 148 months are complete.
# Designed for the one-month-per-execution V3 script.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\run_generate_yearly_totals_chunkedV3_until_done.ps1

$ErrorActionPreference = "Stop"

$PythonExe = "A:\TrafficAnalytics\PROJECTS\env\Scripts\python.exe"
$Script   = "A:\TrafficAnalytics\PROJECTS\scripts\finalscriptsafterdedup\generate_yearly_totals_chunkedV3.py"

$ReportDir = "A:\TrafficAnalytics\PROJECTS\reports\deduped"
$FinalJson = Join-Path $ReportDir "yearly_totals_final.json"
$LogDir    = Join-Path $ReportDir "logs"
$LogFile   = Join-Path $LogDir "yearly_totals_chunkedV3_wrapper.log"
...
Log-Line "Starting yearly totals V3 wrapper"
...
while ($true) {
    ...
    $summary = Get-ProgressSummary
    if ($summary) {
        Log-Line "Progress: $($summary.months_completed)/$($summary.months_total) months complete; best_year=$($summary.best_year); lowest_year=$($summary.lowest_year)"
        if ($summary.zero_row_months) {
            Log-Line "Zero-row months recorded: $($summary.zero_row_months -join ', ')"
        }
