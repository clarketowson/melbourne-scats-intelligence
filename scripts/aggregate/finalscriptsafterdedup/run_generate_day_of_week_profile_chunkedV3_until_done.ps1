# run_generate_day_of_week_profile_chunkedV3_until_done.ps1
#
# Relaunches generate_day_of_week_profile_chunkedV3.py until all 148 months are complete.
# Designed for the one-month-per-execution V3 script.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\run_generate_day_of_week_profile_chunkedV3_until_done.ps1

$ErrorActionPreference = "Stop"

$PythonExe = "A:\TrafficAnalytics\PROJECTS\env\Scripts\python.exe"
$Script   = "A:\TrafficAnalytics\PROJECTS\scripts\finalscriptsafterdedup\generate_day_of_week_profile_chunkedV3.py"

$ReportDir = "A:\TrafficAnalytics\PROJECTS\reports\deduped"
$FinalJson = Join-Path $ReportDir "day_of_week_profile_final.json"
$LogDir    = Join-Path $ReportDir "logs"
$LogFile   = Join-Path $LogDir "day_of_week_profile_chunkedV3_wrapper.log"
...
Log-Line "Starting day-of-week profile V3 wrapper"
...
while ($true) {
    ...
    $summary = Get-ProgressSummary
    if ($summary) {
        Log-Line "Progress: $($summary.months_completed)/$($summary.months_total) months complete; best_day=$($summary.best_day_name); lowest_day=$($summary.lowest_day_name)"
        if ($summary.zero_row_months) {
            Log-Line "Zero-row months recorded: $($summary.zero_row_months -join ', ')"
        }
