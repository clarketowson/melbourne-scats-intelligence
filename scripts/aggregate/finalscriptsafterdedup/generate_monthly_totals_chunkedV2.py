#!/usr/bin/env python3
r"""
generate_monthly_totals_chunkedV2.py

Builds monthly total cleaned volume using the improved one-month-per-execution method.
...
def query_month_total(con: duckdb.DuckDBPyConnection, month: MonthWindow):
    ...
    SELECT SUM(volume_15m) AS month_total_volume
    FROM ranked
    WHERE rn = 1
    """
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    return {
        ...
        "rows_in_monthly_csv": row[0] if row else 0,
        "grand_total_volume": row[1] if row else None,
...
def main() -> None:
    ...
    print("=" * 90)
    print("MONTHLY TOTALS RUN - ONE MONTH PER EXECUTION")
    print("=" * 90)
