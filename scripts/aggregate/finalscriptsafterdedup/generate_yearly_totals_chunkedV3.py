#!/usr/bin/env python3
r"""
generate_yearly_totals_chunkedV3.py

V3 patched yearly totals generator.

Based on generate_yearly_totals_chunkedV2.py, with the key safety fixes from
...
- Uses defensive CSV parsing for existing yearly_totals.csv.
...
MONTHLY_CSV = REPORT_DIR / "yearly_totals.csv"
FINAL_JSON = REPORT_DIR / "yearly_totals_final.json"

DATE_RANGE_START = date(2014, 1, 1)
DATE_RANGE_END = date(2026, 4, 7)

ROW_TYPE_MONTH_TOTAL = "yearly_month_total"
...
def query_yearly_month_summary(con: duckdb.DuckDBPyConnection, month: MonthWindow) -> tuple:
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    worst AS (
        SELECT
            ...
            avg_daily_volume
        FROM agg
        ORDER BY avg_daily_volume ASC, year
        LIMIT 1
    ),
    elapsed AS (
    ...
    SELECT
        (SELECT COUNT(*) FROM monthly) AS rows_in_yearly_totals_csv,
        best.year,
    ...
    if row:
        base_payload.update({
            "rows_in_yearly_totals_csv": row[0] if row[0] is not None else 0,
            "best_year": row[1],
            "best_year_avg_daily_volume": row[2],
            "lowest_year": row[3],
            "lowest_year_avg_daily_volume": row[4],
            "total_elapsed_seconds": round(float(row[5]), 3) if row[5] is not None else 0.0,
        })
...
def print_final_payload(payload: dict) -> None:
    ...
    print(f"Rows in yearly CSV   : {fmt_int(payload['rows_in_yearly_totals_csv'])}", flush=True)
    print(f"Physical CSV rows    : {fmt_int(payload['physical_rows_in_csv'])}", flush=True)
    print(f"Best year so far     : {payload['best_year']} ({fmt_int(payload['best_year_avg_daily_volume'])} avg/day)", flush=True)
    print(f"Lowest year so far   : {payload['lowest_year']} ({fmt_int(payload['lowest_year_avg_daily_volume'])} avg/day)", flush=True)
...
def main() -> None:
    ...
    print("=" * 90, flush=True)
    print("YEARLY TOTALS V3 - ONE MONTH PER EXECUTION", flush=True)
    print("=" * 90, flush=True)
    ...
    try:
        ...
        month_total_volume, days_loaded, avg_daily_volume = query_yearly_month_summary(con, next_month)
        month_elapsed = time.time() - month_start_ts

        year_value = next_month.month_start.year
        ...
        print(f"Year                 : {year_value}", flush=True)
        print(f"Month total volume   : {fmt_int(month_total_volume)}", flush=True)
        print(f"Days loaded          : {fmt_int(days_loaded)}", flush=True)
        print(f"Avg daily volume     : {fmt_int(avg_daily_volume) if avg_daily_volume is not None else 'N/A'}", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print("\nERROR: generate_yearly_totals_chunkedV3.py failed.", file=sys.stderr, flush=True)
