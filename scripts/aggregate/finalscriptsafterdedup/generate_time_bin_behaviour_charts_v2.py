#!/usr/bin/env python3
"""
generate_time_bin_behaviour_charts_v2.py

Derived behavioural analytics, version 2 with standardized annotations for the Melbourne SCATS time-bin profile.
...
Example PowerShell one-liner:
  python generate_time_bin_behaviour_charts_v2.py --csv "A:\TrafficAnalytics\PROJECTS\reports\deduped\time_bin_profile.csv" --json "A:\TrafficAnalytics\PROJECTS\reports\deduped\time_bin_profile_final.json" --out "A:\TrafficAnalytics\PROJECTS\reports\deduped\charts\time_bin_behaviour"
...
def callout_box(
    ...
    text: str,
    xy: tuple | None = None,
    xytext: tuple = (0.05, 0.88),
    colour: str = COLOURS["analysis"],
    fontsize: float = 13.5,
    ha: str = "left",
    va: str = "center",
    ...
    """Standardized annotation/callout styling for all behaviour charts."""
    box = dict(
    ...
    if xy is None or not arrow:
        return ax.text(
            xytext[0],
            xytext[1],
            text,
            transform=ax.transAxes if coords == "axes fraction" else None,
            ha=ha,
            va=va,
            fontsize=fontsize,
            fontweight="bold",
...
def chart_cumulative_daily_curve(d: pd.DataFrame, out_dir: Path) -> Path:
    ...
    for t, label in markers:
        ...
        ax.annotate(
            ...
            bbox=dict(boxstyle="round,pad=0.30", fc="white", ec=COLOURS["border"], alpha=0.96),
        )
...
def chart_morning_vs_afternoon(d: pd.DataFrame, out_dir: Path) -> Path:
    ...
    callout_box(
        ax,
        f"Afternoon pressure is structurally higher\nPeak is {dominance:.2f}× morning-window peak",
        xy=None,
        xytext=(0.50, 0.84),
        colour=COLOURS["peak"],
        fontsize=15.5,
...
def chart_plateau_threshold(d: pd.DataFrame, out_dir: Path) -> Tuple[Path, Dict]:
    ...
    add_title(
        ...
        "Measures how many hours each day Melbourne remains close to maximum network load.",
    )
...
def chart_top_bins_cluster(d: pd.DataFrame, out_dir: Path) -> Tuple[Path, Dict]:
    ...
    cluster_mid = (time_to_minutes(cluster_start) + time_to_minutes(cluster_end)) / 2 / 60
    cluster_peak_y = float(top10["avg_daily_volume"].max())

    callout_box(
        ax,
        f"Top 10 busiest intervals cluster tightly\n{cluster_start} → {cluster_end}  (Span: {cluster_span_hours:.1f} hrs)",
        xy=(cluster_mid, cluster_peak_y),
        xytext=(0.16, 0.84),
        colour=COLOURS["peak"],
        fontsize=14.2,
        ha="left",
