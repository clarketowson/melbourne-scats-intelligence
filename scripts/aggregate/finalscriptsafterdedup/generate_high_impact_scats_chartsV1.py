#!/usr/bin/env python3
r"""
generate_high_impact_scats_chartsV1.py

Third-wave high-impact Melbourne SCATS chart generator.
...
def normalise_monthly(df: pd.DataFrame) -> pd.DataFrame:
    month_col = pick_column(df, ["month_label", "year_month", "month", "month_start", "date_month"], "month")
    vol_col = pick_column(df, ["total_volume", "monthly_volume", "volume", "vehicles", "sum_volume", "month_volume"], "volume")
    out = pd.DataFrame({
        "month": pd.to_datetime(df[month_col].astype(str), errors="coerce"),
        "volume": pd.to_numeric(df[vol_col], errors="coerce"),
    }).dropna()
...
def normalise_daily(df: pd.DataFrame) -> pd.DataFrame:
    date_col = pick_column(df, ["date", "day", "traffic_date", "date_value"], "date")
    vol_col = pick_column(df, ["daily_volume", "total_volume", "volume", "vehicles", "sum_volume"], "volume")
    out = pd.DataFrame({
