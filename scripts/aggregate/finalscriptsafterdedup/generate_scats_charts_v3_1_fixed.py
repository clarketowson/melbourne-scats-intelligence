...
def chart_busiest_sites(df: pd.DataFrame, out_dir: Path, manifest: List[dict]) -> None:
    ...
    top["site_label"] = "Site " + top["site"].astype(str)
    fig, ax = plt.subplots(figsize=(13, 9))
    ax.barh(top["site_label"], top["volume"], color=PALETTE["ranking"])
    x_millions_axis(ax)
    ax.set_xlim(0, top["volume"].max() * 1.10)
    for y, v in enumerate(top["volume"]):
        ax.text(v, y, f" {v/1_000_000:.1f}M", va="center", fontsize=8, color="#27364a")
    ax.text(
        0.01,
        0.01,
        "Axis shown in millions of cleaned vehicle movements. Labels are SCATS site IDs, not volumes.",
        transform=ax.transAxes,
        fontsize=8,
        va="bottom",
        ha="left",
    ...
    manifest.append({"file": path.name, "title": "Top 20 busiest SCATS sites", "source": "chunked_busiest_site_monthly", "axis_fix": "V3.1 formats the x-axis as millions and keeps site IDs as text labels"})
