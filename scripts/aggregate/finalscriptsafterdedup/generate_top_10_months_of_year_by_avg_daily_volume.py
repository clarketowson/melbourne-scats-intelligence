#!/usr/bin/env python3
r"""
generate_top_10_months_of_year_by_avg_daily_volume.py

Builds the top 10 months-of-year by average daily volume from the already-generated
seasonal_chart_ready.csv / seasonal_chart_ready.json.
...
Outputs:
    A:\TrafficAnalytics\PROJECTS\reports\deduped\top_10_months_of_year_by_avg_daily_volume.csv
    A:\TrafficAnalytics\PROJECTS\reports\deduped\top_10_months_of_year_by_avg_daily_volume.json
"""
...
INPUT_CSV = REPORT_DIR / "seasonal_chart_ready.csv"
INPUT_JSON = REPORT_DIR / "seasonal_chart_ready.json"

OUTPUT_CSV = REPORT_DIR / "top_10_months_of_year_by_avg_daily_volume.csv"
OUTPUT_JSON = REPORT_DIR / "top_10_months_of_year_by_avg_daily_volume.json"
...
def write_csv(rows: list[tuple]) -> None:
    with OUTPUT_CSV.open("w", newline="", encoding="utf-8") as f:
        ...
        for rank_position, month_of_year, month_name, total_volume, days_loaded, avg_daily_volume in rows:
            writer.writerow([
                rank_position,
                month_of_year,
                month_name,
...
def main() -> None:
    print("=" * 90)
    print("GENERATING TOP 10 MONTHS OF YEAR BY AVG DAILY VOLUME")
    print("=" * 90)
    ...
    if rows:
        print(f"Top month             : #{rows[0][0]} | {rows[0][2]} | {fmt_int(rows[0][5])}")
    print("=" * 90)
