    ...
    for key, candidates in CSV_CANDIDATES.items():
        if key == "site_lookup":
            ...
            for lookup_path in find_all_site_lookup_tables(input_dir):
                df = read_table_if_exists(lookup_path)
                if df is not None and not df.empty:
                    df = df.copy()
                    df["__lookup_source"] = lookup_path.name
                    lookup_frames.append(df)
                    lookup_paths.append(str(lookup_path))
                    print(f"Loaded site_lookup source: {lookup_path.name} ({len(df):,} rows)")
            if lookup_frames:
...
def chart_busiest_sites(
    ...
    manifest.append({
        ...
        "axis_fix": "V3.4 labels sites with readable names from merged CSV/XLS lookup sources and integer-only site IDs",
    })
