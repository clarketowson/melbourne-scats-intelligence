...
V2 improvement:
  - The known missing month 2018-12 is kept in the timeline but plotted as a
    visual data gap, not as a false zero-traffic plunge.
  - Monthly line charts include a clear "Data gap — 2018-12" annotation.
  - Helper CSVs keep both the raw volume and the charting volume.
...
def annotate_monthly_data_gaps(ax, monthly: pd.DataFrame, y_col: str = "volume_for_chart") -> None:
    ...
    for _, row in gaps.iterrows():
        ax.axvline(row["month"], linestyle="--", linewidth=1.2, alpha=0.65)
        ax.annotate(
            f"Data gap\n{row['month_label']}",
            xy=(row["month"], y_mid),
            xytext=(10, 30),
...
def chart_monthly_totals(monthly: pd.DataFrame, out_dir: Path, manifest: List[dict]) -> None:
    ...
    ax.plot(monthly["month"], monthly["volume_for_chart"], linewidth=2)
    ...
    if chartable["volume_for_chart"].notna().sum() >= 6:
        ...
        fig, ax = plt.subplots(figsize=(14, 7))
        ax.plot(chartable["month"], chartable["volume_for_chart"], linewidth=1.4, label="Monthly total")
        ax.plot(chartable["month"], chartable["rolling_6_month_avg"], linewidth=2.4, label="6-month rolling average")
...
def chart_covid_period(monthly: pd.DataFrame, out_dir: Path, manifest: List[dict]) -> None:
    ...
    ax.plot(covid["month"], covid["volume_for_chart"], linewidth=2)
    billions_axis(ax)
