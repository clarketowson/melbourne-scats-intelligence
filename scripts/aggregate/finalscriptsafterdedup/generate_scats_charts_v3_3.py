...
def normalise_site_lookup(site_lookup: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
    ...
    site_col = None
    for candidate in ["scats site", "scats_site", "site", "site_id", "site no", "site_no"]:
        if candidate in col_map:
            site_col = col_map[candidate]
            break

    name_col = None
    for candidate in ["name", "site name", "site_name", "location", "description"]:
        if candidate in col_map:
    ...
    out = out.drop_duplicates(subset=["site"], keep="first")
    return out[["site", "site_name", "latitude", "longitude"]]
...
def chart_busiest_sites(
    ...
    ax.text(
        0.01,
        0.01,
        f"Labels show SCATS site names plus integer site IDs. {matched}/20 top sites matched to the site lookup table.",
        transform=ax.transAxes,
        fontsize=8,
        va="bottom",
        ha="left",
    ...
    manifest.append({
        ...
        "axis_fix": "V3.3 labels sites with readable names and integer-only site IDs",
    })
