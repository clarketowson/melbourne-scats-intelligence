#!/usr/bin/env python3
r"""
generate_day_of_week_profile_chunkedV2.py

Builds day-of-week traffic profile using the improved one-month-per-execution method.
...
def build_final_result(con: duckdb.DuckDBPyConnection, months: list[MonthWindow]) -> dict:
    ...
    return {
        ...
        "rows_in_day_of_week_profile_csv": row[0] if row else 0,
        "best_day_of_week": row[1] if row else None,
        "best_day_name": row[2] if row else None,
        "best_day_avg_daily_volume": row[3] if row else None,
        "lowest_day_of_week": row[4] if row else None,
        "lowest_day_name": row[5] if row else None,
        "lowest_day_avg_daily_volume": row[6] if row else None,
...
def main() -> None:
    ...
    print("=" * 90)
    print("DAY OF WEEK PROFILE RUN - ONE MONTH PER EXECUTION")
    print("=" * 90)
    ...
    try:
        ...
        month_best_avg = None
        if rows:
            month_best_day = max(rows, key=lambda x: (x[4], -int(x[0])))
        ...
        print(f"Best day this month  : {month_best_day[1] if month_best_day else 'N/A'}")
        print(f"Best avg/day         : {fmt_int(month_best_avg) if month_best_avg is not None else 'N/A'}")
        ...
        print(f"Best day so far      : {final_payload['best_day_name']}")
        print(f"Lowest day so far    : {final_payload['lowest_day_name']}")
