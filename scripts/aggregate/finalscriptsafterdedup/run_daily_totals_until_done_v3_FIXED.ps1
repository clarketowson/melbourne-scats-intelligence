# ============================================================
# RUN DAILY TOTALS V3 UNTIL DONE
# ============================================================
# Wrapper for generate_daily_totals_chunkedV3.py
#
# V3 behavior:
# - Relaunches the Python script until daily_totals_final.json
#   reports is_complete=true.
# - Does not rely on CSV row counts alone.
# - Does not loop forever if the Python script fails.
# - Stops if no month-level progress is detected.
# - Prints current completion status after each run.
# ============================================================

$ErrorActionPreference = "Stop"

$scriptPath = "A:\TrafficAnalytics\PROJECTS\scripts\finalscriptsafterdedup\generate_daily_totals_chunkedV3.py"
$finalJson  = "A:\TrafficAnalytics\PROJECTS\reports\deduped\daily_totals_final.json"

Write-Host "============================================================"
Write-Host "DAILY TOTALS V3 WRAPPER"
Write-Host "============================================================"
...
while ($true) {
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "Launching daily totals V3 run $run"
    Write-Host "============================================================"
    ...
    $busiestDay = $status.busiest_day
    $quietestDay = $status.quietest_day
    $rowsInCsv = $status.rows_in_daily_csv
    $zeroRowMonths = $status.zero_row_months
    ...
    Write-Host "Rows in CSV      : $rowsInCsv"
    Write-Host "Grand total      : $grandTotal"
    Write-Host "Busiest day      : $busiestDay"
    Write-Host "Quietest day     : $quietestDay"
    Write-Host "Zero-row months  : $zeroRowMonths"
    Write-Host "Elapsed seconds  : $elapsed"

    if ($monthsCompleted -eq $lastMonthsCompleted) {
        $noProgressRuns++
    } else {
        $noProgressRuns = 0
        $lastMonthsCompleted = $monthsCompleted
    }

    if ($noProgressRuns -ge $maxNoProgressRuns) {
        Write-Host "No progress detected across $maxNoProgressRuns successful launches."
        Write-Host "Wrapper stopping to avoid an infinite loop. Check the current month query and CSV."
        exit 5
    }
    ...
    if ($isComplete -eq $true) {
        Write-Host ""
        Write-Host "============================================================"
        Write-Host "DAILY TOTALS COMPLETE"
        Write-Host "============================================================"
        Write-Host "Final JSON : $finalJson"
        exit 0
    }
